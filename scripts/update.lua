...
function main()
    ...
    os.cd(name)
    os.exec("git push git@gitlab.com:%s.git dev", repo)
    os.exec("git push git@gitee.com:%s.git dev", (repo:gsub("xmake/", "tboox/")))
    os.exec("git checkout master")
    os.exec("git merge dev")
    os.exec("git push git@github.com:%s.git master", repo)
    os.exec("git push git@gitlab.com:%s.git master", repo)
    os.exec("git push git@gitee.com:%s.git master", (repo:gsub("xmake/", "tboox/")))
end
